...
module.exports = class CSSStore
    ...
    getInlineStyle: (node) ->
        ...
        while i < styleObject.cssProperties.length
    ...
    _removePropertyFromMirror: (mirror, index) ->
        ...
        if mirror.shorthandValues[property.name]
            ...
            while i < properties.length
                properties[i] = null  if properties[i].shorthandName == property.name  if properties[i]
                i++
        newProperties = []
        i = 0
    ...
    _parseProperty: (string) ->
        ...
        try
            ...
            return @_buildMirrorForStyle(testStyleDecl, false)  unless testStyleDecl.cssText == ""
